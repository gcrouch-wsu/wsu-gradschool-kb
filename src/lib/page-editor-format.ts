...
import { indentListItem, listItemFromSelection, outdentListItem } from "@/lib/page-editor-list";
...
export function applyAltText(figure: HTMLElement, alt: string, decorative: boolean): boolean {
  ...
  if (caption) {
    caption.className =
      "doc-image__caption" +
      (decorative ? " doc-image__caption--decorative" : missing ? " doc-image__caption--missing" : "");
    caption.textContent = decorative
      ? "Decorative image (no alt text needed)"
      : value || "No alt text — use the Alt button to add a description";
  }
...
function mutateListItem(li: HTMLLIElement, action: "indent" | "outdent"): boolean {
  ...
  const surface = li.closest(".page-document-editor__surface");
  surface?.dispatchEvent(new InputEvent("input", { bubbles: true }));
  notifyMutation();
