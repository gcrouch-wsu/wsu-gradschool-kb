...
export async function convertDocxToBlocks(
  ...
  if (skippedImages > 0) {
    const count = `${skippedImages} image${skippedImages === 1 ? "" : "s"}`;
    messages.push(
      `${count} could not be imported because the format is not web-compatible — this is usually EMF/WMF (the format Word uses for pasted screenshots). ` +
        `Fix in the source document by deleting and re-inserting each image via Insert → Pictures as PNG or JPG, then re-export and import again. You can also add them later with the image block in the editor.`,
    );
  }
