  ...
  try {
    ...
    const safeUsers = await Promise.all(
      users.map(async ({ passwordHash: _unused, ...user }) => ({
        ...user,
        kbAssignments:
          user.role === "editor" || user.role === "viewer" || user.role === "manager"
            ? await listUserAssignments(user.id)
            : [],
      })),
    );
...
export async function POST(request: Request) {
  ...
  const role: UserRole = ["owner", "admin", "manager", "editor", "viewer"].includes(body.role)
    ? body.role
    : "editor";
  ...
  try {
    ...
    if ((role === "editor" || role === "viewer" || role === "manager") && Array.isArray(body.kbAssignments)) {
